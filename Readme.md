...
This repository contains the data and code for our paper published in **[PNAS](https://www.pnas.org/doi/10.1073/pnas.2418029122)**:
...
- [:file_folder: data](analysis/data):
  ...
  - `raw_data/` — the raw lithic measurements as Excel workbooks:
    - `Longtan_lithic_tools.xlsx` — retouched tools (sheet 1 Quina scrapers, sheet 2 ordinary scrapers, sheet 3 notches/denticulates).
    - `Longtan_lithic_data_flakes.xlsx` — flakes (sheet 1 debitage, sheet 2 resharpening, sheet 3 ordinary, sheet 4 incomplete).
    - `Longtan_lithic_data_cores.xlsx` — complete cores.
    - `Longtan_lithic_data_waste_products.xlsx` — chunks and debris.
    - `Longtan_lithic_data_coordinates.xlsx` — 3D coordinates of stone artifacts.
...
- [:file_folder: scripts](analysis/scripts): all analysis code (R).
  ...
  - [`LT_CODE_MAIN_TEXT.R`](analysis/scripts/LT_CODE_MAIN_TEXT.R) — statistical tests and figures reported in the **main text**.
  - [`LT_CODE_SI.R`](analysis/scripts/LT_CODE_SI.R) — analyses and figures for the **Supplementary Information**.
  - [`LT_CODE_DS.R`](analysis/scripts/LT_CODE_DS.R) — descriptive statistics for the **supplementary datasets**.

- [:file_folder: output](analysis/output): figures generated by the scripts (`figures.pdf` and the 3D plots).
...
# 2. Run it, mounting a host folder so the figures (figures.pdf + the 3-D PNGs)
#    land in ./analysis/output. Pick the volume line that matches your shell:
# bash / macOS / Linux:
docker run --rm -v "$(pwd)/analysis/output:/home/project/analysis/output" longtan-quina
# Windows PowerShell:
docker run --rm -v "${PWD}/analysis/output:/home/project/analysis/output" longtan-quina
# Windows CMD:
docker run --rm -v "%cd%\analysis\output:/home/project/analysis/output" longtan-quina
```

(To run the analysis without copying figures to the host, omit the `-v` option.)
...
-   **Console:** results of the statistical tests and the descriptive statistics tables.
-   **`analysis/output/figures.pdf`:** every main text and SI figure, one page per plot (when run via `run_all.R`).
-   **`analysis/output/3d_plot.png`** and **`analysis/output/3d_plot_filtered_technological_types.png`:** the 3D coordinate plots (from `LT_CODE_SI.R`). The contents of `analysis/output/` are git-ignored.
...
-   **R packages:**
    ...
    | `cowplot` | composing multipanel figures |
    ...
    | `plot3D` | 3D scatter plots of artifact coordinates |
